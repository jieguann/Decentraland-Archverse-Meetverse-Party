...
import { invokeMethod, getAvailableRooms } from './matchmaker/controller.mjs';
    ...
    async handleMatchMakeRequest(req, res) {
        ...
        if (req.method === 'OPTIONS') {
            ...
            req.on('end', async () => {
                ...
                try {
                    const response = await invokeMethod(method, name, clientOptions);
            ...
            headers['Content-Type'] = 'application/json';
            res.writeHead(200, headers);
            res.write(JSON.stringify(await getAvailableRooms(roomName)));
            res.end();
        }
    }
}
